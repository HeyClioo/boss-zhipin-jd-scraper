...
- **可配置的岗位过滤**：以 *AI产品经理* 为例，默认保留**产品/项目经理**岗、剔除**工程师/开发/算法**岗。就一行正则——改它就能保留任意岗位，或完全关闭过滤
- **分轮推进**：收约 30 → 滚动 → 重复；分批抓取带检查点，长任务可续跑、绝不丢进度
...
全部由搜索页 URL 驱动，所以技能支持**任意城市、任意岗位、任意筛选组合**。下面的码只是本 README 用的示例（*AI产品经理 · 北京*）。要拿到**你自己**搜索的码，在浏览器里对 BOSS直聘勾选筛选，直接从生成的 URL 里读即可。
